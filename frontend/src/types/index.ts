...
export type TrainCategory = 
  | 'FR' | 'FA' | 'FB' | 'IC' | 'ICN' 
  | 'EC' | 'EN' | 'REG' | 'RV' | 'MET' | 'OTHER';

export type Operator = 'trenitalia' | 'trenord' | 'italo' | 'other';

export type TrainStatus = 
  | 'scheduled' | 'running' | 'arrived' 
  | 'cancelled' | 'delayed' | 'partially_cancelled';

export type AlertType = 
  | 'strike' | 'disruption' | 'engineering_works' 
  | 'delay' | 'cancellation' | 'other';

// Category styling
export const TRAIN_CATEGORIES: Record<string, { 
  name: string; 
  bgColor: string; 
  textColor: string;
  borderColor?: string;
