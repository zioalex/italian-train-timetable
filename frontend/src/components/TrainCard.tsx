...
export default function TrainCard({ solution, onClick }: TrainCardProps) {
  ...
  const totalDelay = solution.legs.reduce((acc, leg) => {
    return acc + (leg.train.delay || 0);
  }, 0);
  ...
  return (
    ...
    >
      <div className="p-5">
        {/* Main row: category, times, duration */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            {/* Train category badge */}
            <div
              className={`
                ${catStyle.bgColor} ${catStyle.textColor}
                ${catStyle.borderColor ? `border-2 ${catStyle.borderColor}` : ''}
                px-4 py-2 rounded-xl font-bold text-sm min-w-20 text-center
              `}
            >
              {category}
            </div>
            ...
            <div>
              <div className="flex items-center gap-3 text-2xl font-bold text-gray-800">
                <span>{formatTime(solution.departureTime)}</span>
                <span className="text-gray-300 text-lg">→</span>
                <span>{formatTime(solution.arrivalTime)}</span>
              </div>
              <div className="text-sm text-gray-500 mt-1">
                {catStyle.name} {mainTrain?.trainNumber}
                {solution.changes > 0 && (
          ...
          <div className="flex items-center gap-2 text-sm">
            <span
              className={`w-2 h-2 rounded-full ${
                operator === 'trenitalia' ? 'bg-red-500' :
                operator === 'trenord' ? 'bg-green-500' :
                operator === 'italo' ? 'bg-purple-500' : 'bg-gray-500'
              }`}
            />
            <span className={opStyle.color}>{opStyle.name}</span>
          </div>

          {/* Route summary for multi-leg journeys */}
          {solution.legs.length > 1 && (
            <div className="text-sm text-gray-400">
              via {solution.legs.slice(0, -1).map(l => 
                l.destination.name.split(' ')[0]
              ).join(', ')}
            </div>
          )}
